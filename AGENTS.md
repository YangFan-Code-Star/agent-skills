...
**这是框架本身的仓库，不是用它初始化出来的项目。** `.agents/skills/` 是**完整版产品**——五个要被安装到别人项目里的技能；`lite/` 是**轻量版发行包**——两个技能 + 四个骨架文件，供个人 / 中小型项目长期迭代使用。两者里的 `templates/` 都是**产出物**，是别人项目的起点。把它们当成本仓库的配置来改，是这里最容易犯、也最贵的错。
...
| 语法检查 | `node --check .agents/skills/init/scaffold.mjs`、`node --check .agents/skills/init/templates/scripts/audit-context.mjs`、`node --check lite/scaffold.mjs` |
...
1. **不在本仓库运行 `/init`，也不运行不带 `--root` 的 `scaffold.mjs`——完整版和 `lite/scaffold.mjs` 都算。** 前者会把 12 个完整骨架、后者会把轻量骨架写进仓库根，和各自的 `templates/` 树混成两套同名文件，之后没人分得清哪份是产品、哪份是配置。要试脚手架就用 `--dry-run` 或 `--root <临时目录>`。
2. **改 `templates/` 下的文件时，判断标准是「对一个陌生项目是否成立」，不是「对 context-dev 是否方便」。** 那里的每一行都会成为别人项目的起点。
3. **完整版和 lite 两份 `AGENTS.md.tmpl` 的 `.tmpl` 后缀都不能去掉。** 宿主会递归发现并全量注入仓库里所有 `AGENTS.md`，改回 `.md` 就等于把满是初始化占位符、开头写着「这个仓库还没初始化」的模板塞进本仓库每一轮对话。`tests/` 里有用例锁着这条。
4. **根目录不补 `docs/`、`scripts/`、`.agents/evals/`。** 两套骨架只存在于各自的 `templates/` 下。在根目录再放一套会出现两份同名文件，而它们语义完全不同——一份是产出物，一份是配置。
5. **改了任何 `templates/` 或三份脚本，同一次改动里更新 `tests/` 和 `README.md`（改 lite 还要同步 `lite/README.md`）。** README 里有目录结构树和「设计取舍」清单，它是唯一的对外说明；漂了就是对每一个使用者说谎，而这正是这个框架声称要消灭的东西。
...
| 术语 | 含义 | 代码中的名字 |
| --- | --- | --- |
| 完整骨架 | 完整版 scaffold 生成到用户项目根的那 12 个文件 | `SKELETON_FILES`（测试里断言其中 12 个） |
| 轻量骨架 | lite scaffold 生成到用户项目根的那 4 个文件 | `LITE_SKELETON_FILES`（测试里断言其中 4 个） |
...
| 要改轻量版发行包的安装、技能或模板 | [lite/README.md](lite/README.md) |

## 可用技能

这五个是**完整版产品**，不是本仓库的开发工具；`lite/skills/` 下的 `init` / `maintain` 是**轻量版产品**，与完整版二选一安装。在本仓库里它们只有一种正当用法：作为被修改的对象。
...
v0.3.0（`CONTEXT_DEV_VERSION` 在完整版 `scaffold.mjs`、`templates/scripts/audit-context.mjs` 与 `lite/scaffold.mjs` **各有一份，改版本号要三处一起改**）。完整版五个技能、lite 版两个技能齐全。完整版 DSH 正式支持、Codex 只验证安装与 frontmatter；lite 复用同一套 `SKILL.md` 触发机制，尚未单独做宿主端到端验证。
